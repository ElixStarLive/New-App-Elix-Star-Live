...
export async function reverseIapPurchase(
  ...
  return withTransaction(async (client) => {
    ...
    if (unusedCoins > 0) {
      const wallet = await client.query<{ paid_coins: string }>(
        `SELECT paid_coins FROM wallet_balances WHERE user_id = $1 FOR UPDATE`,
        [row.user_id],
      );
      const debit = Math.min(Number(wallet.rows[0]?.paid_coins ?? 0), unusedCoins);
      if (debit > 0) {
