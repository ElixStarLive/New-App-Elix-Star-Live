...
export default function Followers() {
  ...
  const sessionRef = useRef(createFollowersSession());
  const session = sessionRef.current;
  const snap = useFollowersSession(session);
    ...
    <div className="fixed inset-0 z-[100] elix-page-glass bg-transparent flex flex-col max-w-[480px] mx-auto">
      <div className="flex-1 min-h-0 flex flex-col">
        <div className="relative flex items-center justify-between px-3 pb-2 pt-[max(12px,var(--safe-top))]">
          ...
          <h1 className="text-sm font-bold text-[#F5F5F7] absolute left-1/2 -translate-x-1/2">Followers</h1>
          <div className="w-8" />
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-8">
          {snap.phase === "loading" || snap.phase === "idle" ? (
            ...
            <p className="text-white/40 text-sm text-center py-10">No followers yet.</p>
          ) : (
