...
describe("PAGE-058 Privacy ownership", () => {
  it("has one public /privacy owner and the frozen in-app document", () => {
    expect(app.match(/path="\/privacy"/g)?.length).toBe(1);
    expect(app).toMatch(/<Route path="\/privacy" element=\{<Privacy \/>\} \/>/);
    expect(app).not.toMatch(/path="\/legal\/privacy"|path="\/privacy-policy"|path="\/policy"/);
    ...
    expect(page).toMatch(/LEGAL_PRIVACY_SETTINGS_LABEL/);
    ...
    expect(content).not.toMatch(/privacy\.html|PrivacyV2|PrivacyFixed|lorem ipsum|coming soon/);
    expect(content).not.toMatch(/new Date\(/);
  });

  it("does not invent APIs, consent, or PAGE-059", () => {
    expect(page).not.toMatch(/\/api\/privacy|acceptPrivacy|I Agree|mailto:/);
    ...
    expect(terms).not.toMatch(/LEGAL_PRIVACY_TITLE|legalPrivacy/);
    expect(copyright).not.toMatch(/LEGAL_PRIVACY_TITLE|legalPrivacySections|LegalDocPage/);
    expect(copyright).toMatch(/LEGAL_COPYRIGHT_TITLE/);
