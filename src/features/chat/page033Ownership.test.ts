...
describe("PAGE-033 Chat Thread ownership", () => {
  it("owns /inbox/:threadId with one session and inbox message contracts", () => {
    ...
    expect(page).toMatch(/LevelBadge/);
    expect(page).toMatch(/StoryGoldRingAvatar/);
    expect(page).toMatch(/apiLiveStreams/);
    expect(page).toMatch(/firstAppShare/);
    ...
    expect(page).not.toMatch(/new WebSocket|ChatThreadOld|ChatThreadNew|ChatThreadV2|ChatThreadFixed|location\.reload|setTimeout\(/);
    ...
    expect(query).toMatch(/user_engagement/);
