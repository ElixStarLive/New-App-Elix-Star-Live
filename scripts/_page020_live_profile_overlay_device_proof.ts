/**
 * PAGE-020 device proof — /watch/:roomId/profile/:userId overlay chrome.
 * Run: npx tsx scripts/_page020_live_profile_overlay_device_proof.ts
 */
...
async function register(suffix: string) {
  const unique = `d20${suffix}${Date.now().toString(36)}`.slice(0, 16);
...
await sleep(1200);
await evaluate(`(() => { if (!location.pathname.includes('/profile/')) { history.replaceState({}, '', ${JSON.stringify(route)}); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(4000);
...
const proof = (await evaluate(`(() => {
  ...
  return {
    ...
    profileCue: /follow|message|share|followers|following|block|report/i.test(text),
    snippet: text.slice(0, 320),
  };
})()`)) as Record<string, unknown>;

assert(String(proof.pathname || "").includes("/profile/"), `path ${proof.pathname} ${proof.snippet}`);
assert(proof.profileCue === true, `no profile overlay: ${proof.snippet}`);
...
console.log(JSON.stringify({ ok: true, page: "PAGE-020", route, device: devices[0], proof }, null, 2));
