/**
 * PAGE-017 device proof — physical Android /live lobby with local API session.
 * Run: npx tsx scripts/_page017_live_discover_device_proof.ts
 */
...
async function registerSession(): Promise<{ token: string; user: Record<string, unknown> }> {
  const unique = `d17${Date.now().toString(36)}`.slice(0, 16);
...
await sleep(1000);
await evaluate(`(() => { if (location.pathname !== '/live') { history.replaceState({}, '', '/live'); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(3000);
...
const proof = (await evaluate(`(() => {
  ...
  return {
    ...
    liveCue: /live|no live|refresh|watching|stream/i.test(text),
    snippet: text.slice(0, 280),
  };
})()`)) as Record<string, unknown>;

assert(proof.pathname === "/live", `path ${proof.pathname} ${proof.snippet}`);
assert(proof.liveCue === true, `no live lobby UI: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-017", device: devices[0], proof }, null, 2));
