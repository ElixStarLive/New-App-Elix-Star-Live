/**
 * PAGE-016 device proof — physical Android /music with local API session.
 * Run: npx tsx scripts/_page016_music_device_proof.ts
 */
...
async function registerSession(): Promise<{ token: string; user: Record<string, unknown> }> {
  const unique = `d16${Date.now().toString(36)}`.slice(0, 16);
...
await sleep(1000);
await evaluate(`(() => { if (location.pathname !== '/music') { history.replaceState({}, '', '/music'); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(3000);

const proof = (await evaluate(`(() => {
  const text = document.body?.innerText || '';
  const search = !!document.querySelector('input') || text.toLowerCase().includes('search');
  return {
    pathname: location.pathname,
    search,
    musicCue: /music|sound|playlist|track|original/i.test(text),
    snippet: text.slice(0, 280),
  };
})()`)) as Record<string, unknown>;

assert(proof.pathname === "/music", `path ${proof.pathname} ${proof.snippet}`);
assert(proof.search === true || proof.musicCue === true, `no music UI: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-016", device: devices[0], proof }, null, 2));
