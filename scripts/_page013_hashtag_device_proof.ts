/**
 * PAGE-013 device proof — physical Android hashtag route with local API session.
 * Requires: adb device, installDebug build with VITE_API_URL=http://127.0.0.1:8080,
 *           adb reverse tcp:8080, API on :8080.
 * Run: npx tsx scripts/_page013_hashtag_device_proof.ts
 */
...
async function registerSession(): Promise<{ token: string; user: Record<string, unknown>; unique: string }> {
  const unique = `d13${Date.now().toString(36)}`.slice(0, 16);
...
const { token, user, unique } = await registerSession();
console.log("session user:", user.id);

const tag = `d13tag${unique}`.slice(0, 20).toLowerCase();
const { getPool } = await import("../server/infra/postgres.ts");
const pool = getPool();
try {
  await pool.query(
    `INSERT INTO videos (user_id, bunny_path, caption, privacy, hashtags)
     VALUES ($1, $2, $3, 'public', ARRAY[$4])`,
    [user.id, `https://cdn.example/${tag}.mp4`, `device hashtag ${tag}`, tag],
  );
} finally {
...
const route = `/hashtag/${encodeURIComponent(tag)}`;
...
const proof = (await evaluate(`(() => {
  ...
  const title = [...document.querySelectorAll('h1')].some((n) => (n.textContent || '').trim() === ${JSON.stringify(`#${tag}`)});
  const count = /\\d+\\s+videos/.test(text);
  const grid = document.querySelectorAll('img[alt="Video"]').length;
  const empty = text.includes('No videos found for this hashtag');
  const loading = text.includes('Loading...');
  const back = !!document.querySelector('button[title="Back to For You"]');
  return {
...
assert(proof.pathname === route, `pathname ${proof.pathname} snippet=${proof.snippet}`);
assert(proof.title, `missing #tag title: ${proof.snippet}`);
assert(proof.back, "missing back control");
assert(!proof.loading, `still loading: ${proof.snippet}`);
assert((proof.grid ?? 0) > 0 && proof.count, `expected seeded video on #${tag}: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-013", tag, device: devices[0], proof }, null, 2));
