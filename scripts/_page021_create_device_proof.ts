/**
 * PAGE-021 device proof — physical Android /create camera chrome.
 * Run: npx tsx scripts/_page021_create_device_proof.ts
 */
...
async function registerSession() {
  const unique = `d21${Date.now().toString(36)}`.slice(0, 16);
...
await sleep(1200);
await evaluate(`(() => { if (location.pathname !== '/create') { history.replaceState({}, '', '/create'); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(3000);
...
const proof = (await evaluate(`(() => {
  ...
  return {
    ...
    createCue: /upload|live|post|create|add sound|sound|camera|flip|effects/i.test(text),
    video: !!document.querySelector('video'),
    snippet: text.slice(0, 320),
  };
})()`)) as Record<string, unknown>;

assert(proof.pathname === "/create", `path ${proof.pathname} ${proof.snippet}`);
assert(proof.createCue === true, `no create chrome: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-021", device: devices[0], proof }, null, 2));
