/**
 * PAGE-015 device proof — physical Android /saved with local API session.
 * Run: npx tsx scripts/_page015_saved_device_proof.ts
 */
...
async function registerSession(): Promise<{ token: string; user: Record<string, unknown> }> {
  const unique = `d15${Date.now().toString(36)}`.slice(0, 16);
...
let videoId = "";
try {
  const vid = await pool.query<{ id: string }>(
    `INSERT INTO videos (user_id, bunny_path, caption, privacy)
     VALUES ($1, $2, 'saved device', 'public') RETURNING id`,
    [user.id, `https://cdn.example/d15-${user.id}.mp4`],
  );
  videoId = vid.rows[0].id;
  await pool.query(`INSERT INTO video_saves (user_id, video_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [
    user.id,
    videoId,
  ]);
} finally {
...
await sleep(1000);
await evaluate(`(() => { if (location.pathname !== '/saved') { history.replaceState({}, '', '/saved'); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(2500);

const proof = (await evaluate(`(() => {
  const text = document.body?.innerText || '';
  const title = [...document.querySelectorAll('h1')].some((n) => (n.textContent || '').includes('Saved'));
  return {
    pathname: location.pathname,
    title,
    empty: text.includes('No saved videos yet'),
    media: document.querySelectorAll('img, video').length,
    snippet: text.slice(0, 240),
  };
})()`)) as Record<string, unknown>;

assert(proof.pathname === "/saved", `path ${proof.pathname}`);
assert(proof.title === true, `no Saved title: ${proof.snippet}`);
assert(proof.empty !== true, `empty despite seed: ${proof.snippet}`);
assert((proof.media as number) > 0, `no media cells: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-015", videoId, device: devices[0], proof }, null, 2));
