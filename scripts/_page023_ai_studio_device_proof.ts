/**
 * PAGE-023 device proof — physical Android /ai-studio chrome.
 * Run: npx tsx scripts/_page023_ai_studio_device_proof.ts
 */
...
async function registerSession() {
  const unique = `d23${Date.now().toString(36)}`.slice(0, 16);
...
assert(
  ...
  history.replaceState({}, '', '/ai-studio');
...
await sleep(1200);
await evaluate(`(() => { if (location.pathname !== '/ai-studio') { history.replaceState({}, '', '/ai-studio'); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(3000);
...
const proof = (await evaluate(`(() => {
  ...
  return {
    ...
    aiCue: /ai studio|filters|enhance|export|import|tools|captions|thumbnail|background/i.test(text),
    snippet: text.slice(0, 320),
  };
})()`)) as Record<string, unknown>;

assert(proof.pathname === "/ai-studio", `path ${proof.pathname} ${proof.snippet}`);
assert(proof.aiCue === true, `no ai studio chrome: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-023", device: devices[0], proof }, null, 2));
