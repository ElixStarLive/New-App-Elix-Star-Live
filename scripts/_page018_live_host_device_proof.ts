/**
 * PAGE-018 device proof — physical Android host go-live route chrome.
 * Run: npx tsx scripts/_page018_live_host_device_proof.ts
 */
...
async function registerSession(): Promise<{ token: string; user: Record<string, unknown> }> {
  const unique = `d18${Date.now().toString(36)}`.slice(0, 16);
...
await sleep(1200);
await evaluate(`(() => { if (!location.pathname.startsWith('/live')) { history.replaceState({}, '', ${JSON.stringify(route)}); window.dispatchEvent(new PopStateEvent('popstate')); } return location.pathname; })()`);
await sleep(4000);
...
const proof = (await evaluate(`(() => {
  ...
  return {
    ...
    hostCue: /live|join|gift|camera|mic|end|connecting|permission/i.test(text),
    snippet: text.slice(0, 320),
  };
})()`)) as Record<string, unknown>;

assert(String(proof.pathname || "").startsWith("/live"), `path ${proof.pathname} ${proof.snippet}`);
assert(proof.hostCue === true, `no host live chrome: ${proof.snippet}`);

console.log(JSON.stringify({ ok: true, page: "PAGE-018", route, device: devices[0], proof }, null, 2));
